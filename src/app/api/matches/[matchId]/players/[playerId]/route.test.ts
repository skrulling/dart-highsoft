...
describe('DELETE /api/matches/[matchId]/players/[playerId]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('returns 403 for tournament matches', async () => {
    ...
    const request = new Request('http://localhost/api/matches/match-1/players/player-1', {
      method: 'DELETE',
    });

    const response = await DELETE(request, { params: Promise.resolve({ matchId: 'match-1', playerId: 'player-1' }) });
