-- Restore inverted multiplayer Elo K-factor scaling after 0040 recreated the
-- function for idempotency and unintentionally brought back the previous
-- shrinking-field formula.
--
-- Desired behavior: larger multiplayer fields should produce larger Elo swings.
-- Normalization keeps 3-player matches at the base K-factor.
...
begin
  ...
  -- Normalized so that 3 players (sqrt(2) / sqrt(2)) uses base K.
